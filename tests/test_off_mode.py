...
class ConfigValidationTests(unittest.TestCase):
    ...
    def test_rejects_invalid_power_target_during_save(self):
        ...
        self.assertEqual(
            validate_config_patch(config),
            "Eco Power-Ziel muss eine Zahl sein",
        )


class RejectingBraiins:
    def get_status(self):
        return {
            "power_watt": 1000,
            "paused": False,
...
class AutoPauseRuleTests(unittest.TestCase):
    def setUp(self):
        self.tempdir = tempfile.TemporaryDirectory()
        self.config = ConfigManager(str(Path(self.tempdir.name) / "config.json"))
        self.config.update({
            ...
            "miner": {"host": "miner.test"},
            "control": {
                "enable_pause_battery_discharge": True,
                "pause_battery_discharge_watt": 300,
                "enable_pause_grid_import": True,
                "pause_grid_import_watt": 300,
                "stop_stable_minutes": 0,
            },
        })
    ...
    def controller(self, braiins=None):
        return PowerController(
            self.config,
            FakeFronius(self.powerflow),
            braiins or ForbiddenBraiins(),
            StateStore(),
        )

    def test_both_watt_rules_trigger_at_the_configured_boundary(self):
        controller = self.controller()
        controller._cur_action = "run"

        desired = controller._decide_auto(self.powerflow, self.config.get(), 1000)

        self.assertEqual(desired.action, "pause")
        self.assertTrue(desired.immediate_pause)
        self.assertIn("Akku entlädt", desired.reason)
        self.assertIn("Netzbezug", desired.reason)

    def test_watt_rule_prevents_restart_while_condition_is_active(self):
        self.config.update({"control": {"stop_stable_minutes": 2}})
        controller = self.controller()
        controller._cur_action = "pause"

        desired = controller._decide_auto(self.powerflow, self.config.get(), 0)

        self.assertEqual(desired.action, "pause")
        self.assertEqual(controller._auto_gate(desired.action, self.config.get()), "pause")

    def test_watt_rules_are_not_hidden_by_battery_reserve_branch(self):
        self.config.update({
            "control": {
                "enable_pause_soc": True,
                "pause_soc_percent": 80,
            },
        })
        controller = self.controller()
        controller._cur_action = "run"
        ...
        self.assertEqual(desired.title, "Schutzregel aktiv")
        self.assertIn("Netzbezug", desired.reason)

    def test_two_minute_grace_then_pauses(self):
        self.config.update({"control": {"stop_stable_minutes": 2}})
        controller = self.controller()
        controller._cur_action = "run"
        desired = controller._decide_auto(self.powerflow, self.config.get(), 1000)

        self.assertFalse(desired.immediate_pause)
        self.assertEqual(
            controller._auto_gate(desired.action, self.config.get(), immediate_pause=False),
            "run",
        )
        controller._stop_since -= 121
        self.assertEqual(
            controller._auto_gate(desired.action, self.config.get(), immediate_pause=False),
            "pause",
        )

    def test_failed_pause_keeps_observed_mining_state_in_ui(self):
        state = StateStore()
        controller = PowerController(
            self.config,
            FakeFronius(self.powerflow),
        ...
        snapshot = state.snapshot()
        self.assertEqual(snapshot["display_state"], "mining")
        self.assertEqual(snapshot["command_state"], "failed")


class FroniusValidationTests(unittest.TestCase):
    def test_rejects_missing_or_non_finite_power_values(self):
        with self.assertRaises(ValueError):
            FroniusAPI._site_number({"P_Grid": None}, "P_Grid")
        with self.assertRaises(ValueError):
            FroniusAPI._site_number({"P_Grid": float("nan")}, "P_Grid")

    def test_rejects_invalid_soc(self):
        self.assertIsNone(FroniusAPI._first_soc({"1": {"SOC": float("nan")}}))
        self.assertIsNone(FroniusAPI._first_soc({"1": {"SOC": 101}}))
        self.assertEqual(FroniusAPI._first_soc({"1": {"SOC": 55.5}}), 55.5)


class AdditionalConfigValidationTests(unittest.TestCase):
    def test_zero_minute_pause_delay_is_valid(self):
        config = ConfigValidationTests.valid_config()
        config["control"]["stop_stable_minutes"] = 0
        self.assertIsNone(validate_config_patch(config))

    def test_rejects_non_boolean_rule_toggle(self):
        config = ConfigValidationTests.valid_config()
        config["control"]["enable_pause_grid_import"] = "false"
        normalize_config_patch(config, repair_invalid=False)
        self.assertEqual(
            validate_config_patch(config),
            "enable_pause_grid_import muss true oder false sein",
        )

    def test_rejects_non_string_host(self):
        config = ConfigValidationTests.valid_config()
        config["fronius"]["host"] = 123
        self.assertEqual(validate_config_patch(config), "Fronius-IP muss Text sein")
